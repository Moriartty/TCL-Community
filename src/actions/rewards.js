...
actions.loadAvailableRewards = () => dispatch => {
...
actions.loadFirstPage = () => (dispatch,getState) => {
    dispatch({type:'REWARDS_LIST_LOADING',refreshState:RefreshState.HeaderRefreshing});
    setTimeout(function(){
        const preData = getState().rewards.availableRewards;
...
actions.loadNextPage = () => (dispatch,getState) => {
    dispatch({type:'REWARDS_LIST_LOADING',refreshState:RefreshState.FooterRefreshing});
    setTimeout(function(){
        const preData = getState().rewards.availableRewards;
