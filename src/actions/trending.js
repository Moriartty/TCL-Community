...
actions.loadData = () => dispatch => {
    dispatch({type:'TRENDING_LIST_LOADING',refreshState:RefreshState.HeaderRefreshing});
    setTimeout(function(){
        let mockData = [];
        for(let i=0;i<10;i++){
            mockData.push( {id: i, imageUrls: randomGetPics(), title: 'winter is coming'+i})
        }
        dispatch({type:'TRENDING_LIST_DATA_LOAD',data:mockData});
        dispatch({type:'TRENDING_LIST_LOADING',refreshState:RefreshState.Idle});
    },1000);
};
actions.loadFirstPage = () => (dispatch,getState) => {
    ...
    setTimeout(function(){
        ...
        dispatch({type:'TRENDING_LIST_DATA_LOAD',data:temp.concat(preData)});
        dispatch({type:'TRENDING_LIST_LOADING',refreshState:RefreshState.Idle});
    },1000);
};

actions.loadNextPage = () => (dispatch,getState) => {
    dispatch({type:'TRENDING_LIST_LOADING',refreshState:RefreshState.FooterRefreshing});
    setTimeout(function(){
        const preData = getState().trending.data;
        ...
        dispatch({type:'TRENDING_LIST_DATA_LOAD',data:preData.concat(temp)});
        dispatch({type:'TRENDING_LIST_LOADING',refreshState:RefreshState.Idle});
    },1000);
};
